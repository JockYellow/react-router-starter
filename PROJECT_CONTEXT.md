...
- Static content for blog category metadata is stored under `app/content`; blog/changelog runtime data is served from D1.
...
- `app/content/`: legacy JSON content and category metadata (migration input source).
...
- `toolbelt/`: Local helper API service (concert scrape + git ops). No local admin HTML UI.
...
- Blog content: categories from `app/content/blog/categories.json`; posts are managed in D1. Summary auto-derived when missing.
- Changelog content: frontend `/changelog`, homepage snippets, and `/admin/changelog` all use D1 as the single source of truth.
...
## Local Dev Endpoints
- Main app (Wrangler dev): `http://127.0.0.1:8787`.
- Toolbelt API: `http://127.0.0.1:43210` (`/key`, `/ops/git/commit-push`, `/ops/concert-events/scrape`).
- Git admin UI entry: `/admin/ops` on the main app (local only).
