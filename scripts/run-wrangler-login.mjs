...
try {
  ...
  console.error("Failed to temporarily move .env for wrangler login.");
  console.error(error);
  process.exit(1);
}
...
const args = ["login", ...process.argv.slice(2)];
...
const cleanup = () => {
  try {
    ...
    console.error("Failed to restore .env after wrangler login.");
    console.error(error);
  }
};
