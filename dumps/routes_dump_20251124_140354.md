...
type SectionKey = "blog" | "changelog" | "outerspace";

const EXPERIMENTS = [
  {
    title: "OuterSpace Senter",
    description: "全螢幕 3D 儀表板，使用 Three.js 呈現地球與即時遙測感。",
    path: "/outerspace/OuterSpaceSenter",
    meta: "視覺實驗 / Three.js",
  },
  {
    title: "XENO-DB",
    description: "呼叫 PokeAPI 的資料庫介面，雷達圖與叫聲播放都串在一起。",
    path: "/outerspace/PokeAPIcreatures",
    meta: "資料串接 / PokeAPI",
  },
  {
    title: "Gravity Lab",
    description: "行星重力模擬，拖曳方塊感受不同 g 值與軌跡衰減。",
...
> = {
  blog: {
    ...
    hero: {
      eyebrow: "Blog",
      title: "最新文章 · 兩層分類的筆記流",
      description: "直接讀取 D1 內的真實文章，分類與摘要都來自資料庫，沒有示範假內容。",
      background: "radial-gradient(circle at 15% 20%, #fff5e5 0%, #f1e8ff 35%, #e9f7ff 70%)",
      foreground: "linear-gradient(140deg, rgba(255,255,255,0.9), rgba(255,255,255,0.7))",
      accent: "#6b5ca0",
      cta: { label: "開啟文章列表", to: "/blog" },
      secondary: { label: "查看全部分類", to: "/blog" },
      notes: ["真實文章內容", "兩層分類", "摘要來自正文"],
    },
  },
  changelog: {
    label: "Changelog",
    hero: {
      eyebrow: "Changelog",
      title: "最近的版本節奏",
      description: "更新日誌同樣取自內容檔案，按日期排序，不會顯示不存在的變更。",
      background: "radial-gradient(circle at 80% 10%, #e8fff5 0%, #f7f9ff 55%, #fff7ea 90%)",
      foreground: "linear-gradient(150deg, rgba(255,255,255,0.92), rgba(255,255,255,0.75))",
      accent: "#1f7a4d",
      cta: { label: "檢視更新日誌", to: "/changelog" },
      secondary: null,
      notes: ["實際內容檔案", "日期排序", "標籤加註"],
    },
  },
  outerspace: {
    label: "OuterSpace",
    hero: {
      eyebrow: "OuterSpace",
      title: "實驗索引（不限太空主題）",
      description: "集中入口直達每個實驗頁面，全都是真實存在的連結，不放示範卡片。",
      background: "radial-gradient(circle at 30% 30%, #e6f3ff 0%, #f4f2ff 40%, #fef7ff 75%)",
      foreground: "linear-gradient(160deg, rgba(255,255,255,0.9), rgba(255,255,255,0.65))",
      accent: "#0ea5e9",
      cta: { label: "前往實驗索引", to: "/outerspace" },
      secondary: null,
      notes: ["全螢幕 / 半螢幕實驗", "含返回按鈕", "即時互動"],
    },
  },
};

export async function loader(args: LoaderFunctionArgs) {
  const [changelog, categories] = await Promise.all([loadChangelogItems(args), loadBlogCategories()]);
  const db = requireBlogDb(args.context);
  const posts: BlogPost[] = await getAllBlogPosts(db);
  return { changelog, posts, categories } satisfies LoaderData;
}
...
function categoryLabel(post: BlogPost, categories: BlogCategory[]) {
...
function TagBadge({ tag }: { tag?: ChangelogItem["tag"] }) {
  if (!tag) return null;
  const labels = { add: "新增", fix: "修正", change: "變更", docs: "文件" } as const;
  return <span className="chip">{labels[tag]}</span>;
}
  ...
  return (
    <div className="sticky-tabs-shell">
      <div className="mx-auto max-w-6xl px-4 py-3 space-y-1">
        ...
        <div className="sticky-tabs flex flex-wrap">
              ...
              <button
                ...
                ${
                  isActive
                    ...
                    : "bg-white/80 text-neutral-600 border-white/70 hover:text-neutral-900"
                }`}
              >
...
function HeroLayer({ accent }: { accent: string }) {
  return (
    <>
      <div className="absolute -left-20 -top-16 h-48 w-48 rounded-full blur-3xl opacity-70 animate-pulse" style={{ background: accent }} />
      <div className="absolute right-0 top-1/2 h-64 w-64 -translate-y-1/2 rounded-full blur-3xl opacity-60 animate-pulse" style={{ background: `${accent}55` }} />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.9),transparent_45%)] pointer-events-none" />
    </>
  );
}

export default function HomePage() {
  const { changelog, posts, categories } = useLoaderData() as LoaderData;
  const [activeSection, setActiveSection] = useState<SectionKey>("blog");

  const latestPosts = useMemo(() => posts.slice(0, 6), [posts]);
  const latestChangelog = useMemo(() => changelog.slice(0, 5), [changelog]);

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, [activeSection]);

  const hero = SECTIONS[activeSection].hero;
      ...
      return latestPosts.length ? (
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {latestPosts.map((post) => {
            ...
            const category = categoryLabel(post, categories);
            return (
              <Link
                key={(post.slug ?? post.title) + post.publishedAt}
                to={`/blog/${post.slug}`}
                className="card hover-raise h-full flex flex-col gap-3 animate-room-in"
              >
                <div className="flex items-center justify-between text-xs uppercase tracking-[0.24em] text-neutral-500">
                  <span>{formatDate(post.publishedAt)}</span>
                  <span className="rounded-full bg-[color:rgba(106,96,160,0.1)] px-3 py-1 text-[10px] font-semibold text-[--color-accent-500]">
                    {category}
                  </span>
                </div>
                <h3 className="text-lg font-semibold text-neutral-900">{post.title}</h3>
                {excerpt ? <p className="text-sm text-neutral-700 leading-relaxed flex-1">{excerpt}</p> : null}
                <span className="text-sm font-semibold text-[--color-accent-500]">閱讀文章 →</span>
              </Link>
            );
          })}
        </div>
      ) : (
        <p className="text-sm text-neutral-600">目前沒有文章，從後台新增第一篇吧。</p>
      );
    }

    if (activeSection === "changelog") {
      return latestChangelog.length ? (
        <div className="space-y-4">
          {latestChangelog.map((log, index) => (
            <article
              key={(log.filename ?? log.date) + index}
              className="card hover-raise space-y-2 animate-room-in"
            >
              ...
              <h3 className="text-lg font-semibold text-neutral-900">{log.title}</h3>
              {log.notes?.length ? (
        ...
        <p className="text-sm text-neutral-600">還沒有日誌，去 /changelog 查看詳情或新增內容。</p>
      );
    }
    ...
    return (
      <div className="grid gap-5 md:grid-cols-2">
        {EXPERIMENTS.map((item) => (
          <article key={item.path} className="card hover-raise h-full flex flex-col gap-3 animate-room-in">
            <div className="text-xs uppercase tracking-[0.24em] text-neutral-500">{item.meta}</div>
            <h3 className="text-lg font-semibold text-neutral-900">{item.title}</h3>
            <p className="text-sm text-neutral-700 leading-relaxed flex-1">{item.description}</p>
            <Link to={item.path} className="btn-ghost w-fit">
              進入實驗 →
            </Link>
          </article>
        ))}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 space-y-12">
      <div className="sticky top-0 z-30 -mx-4">
        <ModuleButtons activeSection={activeSection} onChange={setActiveSection} />
      </div>

      <section className="section pt-4">
        <div
          key={activeSection}
          className="module-panel module-hero space-y-8 animate-room-in relative overflow-hidden"
          style={{ background: hero.background, borderColor: `${hero.accent}55` }}
        >
          <HeroLayer accent={hero.accent} />
          <div className="space-y-6 relative z-10">
            <div className="space-y-3">
              <p className="eyebrow">{hero.eyebrow}</p>
              <h1 className="text-3xl md:text-5xl font-bold leading-tight text-neutral-900">{hero.title}</h1>
              <p className="text-lg text-neutral-800/90 leading-relaxed max-w-3xl">{hero.description}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link to={hero.cta.to} className="btn-primary">
                {hero.cta.label}
              </Link>
              {hero.secondary ? (
                <Link to={hero.secondary.to} className="btn-ghost">
                  {hero.secondary.label}
                </Link>
              ) : (
                <Link to="/outerspace" className="btn-ghost">
                  OuterSpace 索引
                </Link>
              )}
            </div>
            <div className="flex flex-wrap gap-2">
              {hero.notes.map((note) => (
                <span
                  key={note}
                  className="stat-pill shadow-sm bg-white/80 text-neutral-800 border-white/70 animate-room-in"
                  style={{ borderColor: `${hero.accent}33` }}
                >
                  {note}
                </span>
              ))}
          ...
          <div
            className="absolute inset-y-6 right-6 w-1/3 min-w-[180px] rounded-[28px] border border-white/70 shadow-2xl bg-white/50 backdrop-blur-md hidden md:block"
            style={{ background: hero.foreground }}
          >
            <div className="h-full w-full bg-gradient-to-b from-white/70 via-transparent to-white/20 rounded-[28px]" />
          </div>
        </div>
      </section>

      <section className="section pt-0">
        <div className="module-panel animate-room-in" style={{ "--module-color": "#ffffff" } as CSSProperties}>
          <div className="space-y-6">
            <div className="space-y-2">
              <p className="eyebrow">{SECTIONS[activeSection].hero.eyebrow}</p>
              <h2 className="section-title">真實內容</h2>
              <p className="text-neutral-700">
                {activeSection === "blog"
                  ? "以下卡片直接來自資料庫文章。"
                  : activeSection === "changelog"
                    ? "更新日誌來自內容檔案，按日期排序。"
                    : "實驗入口都指向真實的 OuterSpace 頁面。"}
              </p>
            </div>
            {renderSection()}
          </div>
