...
import { searchBangumiAnime, type BangumiAnimeCandidate } from "./providers/bangumi.server";
...
async function loadTitleContext(db: D1Database, animeId: number) {
  const catalog = await db
    .prepare(
      `SELECT anime_id, title_zh_tw, title_native, title_romaji, title_english, year
       FROM anime_items
       WHERE anime_id = ?`,
    )
    .bind(animeId)
  ...
  const aliases = await db
    .prepare("SELECT alias FROM anime_item_aliases WHERE anime_id = ? ORDER BY is_primary DESC, id ASC")
    .bind(animeId)
    .all<AliasRow>();

  return { catalog, aliases: (aliases.results ?? []).map((row) => row.alias) };
}
...
async function persistChineseTitle(
  ...
  const statements = [
    db
      ...
      .bind(titleZhTw, candidate.id, now, animeId),
  ];

  const aliases = [
    { alias: candidate.nameCn, source: `bangumi:${candidate.id}:name_cn`, language: "zh-CN", isPrimary: 0 },
    { alias: titleZhTw, source: "resolved:zh-tw", language: "zh-TW", isPrimary: 1 },
  ...
  for (const alias of aliases) {
    ...
    statements.push(
      db
        ...
        .bind(animeId, alias.alias, normalized, alias.source, alias.language, alias.isPrimary, now),
    );
  }
...
export async function ensureAnimeChineseTitle(
  ...
  const loaded = await loadTitleContext(db, animeId);
  ...
  await persistChineseTitle(db, animeId, candidate, titleZhTw);
  return { status: "ENRICHED", titleZhTw, bangumiId: candidate.id, sourceTitle: candidate.nameCn };
}
