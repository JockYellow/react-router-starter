import { PROFILE, PROFILE_VERSION } from "../../data/profile";
...
function serializeProfile(): string {
  const compactProfile = {
    ...
    jobDirections: PROFILE.jobDirections,
    ...
    factRules: PROFILE.factRules,
  };
  return JSON.stringify(compactProfile);
}

export const STABLE_PROFILE_PREFIX = `
...
## 證據使用方式
- 每個重要適配判斷都要盡量連回 Profile 的具體經歷、highlight、detail、story、project、skill 或已確認數字。
...
## 公司適配分析模式
輸出固定依序包含下列七段，不可省略：
1. 「一句話結論」：一至兩句，明確說明適配程度與主要依據，不使用虛構百分比或評分。
2. 「適配原因」：列出最關鍵的職能交集，避免重複。
3. 「經歷佐證」：逐點連結到 Profile 的具體工作或專案證據。
4. 「公司／產業連結」：所有公司事實都須標示【使用者提供】或【推論】。
5. 「可帶來的價值」：描述能解決的問題與工作方式，不承諾未經證實的業績。
6. 「待確認或補強」：指出資訊缺口、面試應確認事項與真實能力邊界。
7. 「自薦摘要」：可直接用於應徵的精簡第一人稱段落，只能使用 Profile 事實。

## 履歷聊天模式
- 回答招募者對候選人經歷、技能、成果、工作方式、求職方向與作品的問題。
- 先給直接答案，再用少量具體證據支持；若問題橫跨多段經歷，可整理共同能力但要保留來源脈絡。
- 不代替候選人承諾到職日、薪資、出勤、工時、搬遷或尚未確認的合作條件。
- 若問題不是履歷相關，簡短說明此助理只回答候選人履歷與職涯資訊，並邀請對方改問相關問題。
...
export function assertPromptCacheVersion(promptCacheKey: string): void {
  if (promptCacheKey !== PROFILE_VERSION) {
    throw new AIError("SERVER_MISCONFIGURED", 503);
  }
}

function stablePrefixItem(): OpenAIInputItem {
  return {
    type: "message",
    role: "developer",
    content: [
  ...
  return [
    {
      type: "message",
      role: "developer",
      content: "目前模式：公司適配分析。嚴格依固定七段格式輸出，總長控制在約 1,000 tokens 內。",
    },
  ...
  return [
    {
      type: "message",
      role: "developer",
      content: "目前模式：履歷聊天。回答最近一則使用者問題，總長控制在約 700 tokens 內。",
    },
    ...messages.map((message): OpenAIInputItem => ({
      type: "message",
      role: message.role,
...
export function buildOpenAIInput(feature: AIFeature, input: CompanyFitInput | ChatInput): OpenAIInputItem[] {
  const dynamicItems = feature === "company-fit"
    ? companyFitItems(input as CompanyFitInput)
    : chatItems((input as ChatInput).messages);
  return [stablePrefixItem(), ...dynamicItems];
}
