...
export default function RngPromptRoute() {
  ...
  const getRandomItems = (pool: PromptItem[], count: number) => {
    if (!pool || pool.length === 0) return [];
    const activePool = pool.filter((item) => item.is_active !== false);
    const shuffled = [...activePool].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, count);
  };

  const rollCategory = (catSlug: string) => {
    const catConfig = configData.find((cat) => cat.slug === catSlug);
  ...
  const toggleLock = (slug: string) => setLocks((prev) => ({ ...prev, [slug]: !prev[slug] }));

  const toggleCategoryActive = (slug: string) => {
    const current = draws[slug];
    if (current && current.length > 0) setDraws((prev) => ({ ...prev, [slug]: [] }));
  ...
  const generatedPrompt = configData
    .map((cat) => draws[cat.slug]?.map((item) => item.value).join(", "))
    .filter(Boolean)
    .join(", ");
      ...
      <div className="sticky top-0 z-40 bg-white shadow-sm border-b border-slate-200">
        <div className="max-w-4xl mx-auto p-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={rollAll}
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md flex items-center justify-center gap-2 shadow-sm active:scale-95 transition-transform"
            >
              <Dice5 size={18} /> 隨機生成
            </button>
            <button
              onClick={copyPrompt}
              className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-md flex items-center justify-center gap-2 shadow-sm active:scale-95 transition-transform"
            >
              <Copy size={18} /> 複製
            </button>
          </div>
      ...
      <div className="max-w-4xl mx-auto p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {configData.map((cat) => {
          ...
          return (
            <div
              key={cat.id}
              className={`flex flex-col rounded-xl border ${cardBorder} bg-white shadow-sm transition-all duration-200 overflow-hidden`}
            >
              <div
              ...
              >
                <div className="flex items-center gap-2 overflow-hidden">
                  {isOptional && (
                    <div
                      className={`w-3 h-3 rounded border flex items-center justify-center ${
                        hasContent ? "bg-pink-500 border-pink-500" : "border-slate-300 bg-white"
                      }`}
                    >
                  ...
                  <span className={`text-[9px] px-1 py-0.5 rounded font-bold uppercase ${badgeColor}`}>
                    {cat.type === "group" ? "GRP" : cat.type === "optional" ? "OPT" : "REQ"}
                  </span>
                  <span
                    className={`text-sm font-bold truncate ${
                      !hasContent && isOptional ? "text-slate-400" : "text-slate-700"
                    }`}
                  >
                    {cat.label}
                  </span>
              ...
              <div
                className={`p-2 flex-1 min-h-[50px] flex flex-wrap content-start gap-1 cursor-pointer hover:bg-slate-50 transition-colors ${
                  !hasContent ? "opacity-50" : ""
                }`}
                onClick={() => {
                  if (!isLocked) rollCategory(cat.slug);
                }}
              >
                {hasContent ? (
                  draws[cat.slug].map((item, idx) => (
                    <span
                      key={`${item.id}-${idx}`}
              ...
              <div className="flex border-t border-slate-100 divide-x divide-slate-100">
                <button
                  ...
                  className={`flex-1 py-2 flex items-center justify-center text-xs font-medium ${
                    isLocked ? "bg-amber-100 text-amber-700" : "text-slate-400 hover:bg-slate-50"
                  }`}
                >
                ...
                <button
                  onClick={(event) => {
                    event.stopPropagation();
                    rollCategory(cat.slug);
                  }}
                  disabled={isLocked}
                  className="flex-1 py-2 flex items-center justify-center text-xs font-medium text-slate-400 hover:bg-slate-50 disabled:opacity-30"
                >
                  <RefreshCw size={12} />
                </button>
              </div>
